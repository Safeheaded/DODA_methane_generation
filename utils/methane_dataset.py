...
class MethaneUnconditionaltBase(Dataset):
    ...
    def __getitem__(self, i):
        ...
        allowed_inputs = ["TOA_AVIRIS_460nm.tif", "TOA_AVIRIS_550nm.tif", "TOA_AVIRIS_640nm.tif"]
        
        inputs_paths = [f for f in path.iterdir() if f.is_file() and f.name not in excluded_inputs and f.name in allowed_inputs]
        inputs_paths.sort()
        ...
        try:
            resized_channels = [
                cv2.resize(image[:, :, i], (self.size, self.size), interpolation=cv2.INTER_LINEAR)
                for i in range(image.shape[2])
            ]
            image = np.stack(resized_channels, axis=-1)
        except:
            resized_channels = [
                cv2.resize(image_cp[:, :, i], (self.size, self.size), interpolation=cv2.INTER_LINEAR)
...
class methaneUnconditionalTrain(MethaneUnconditionaltBase):
    def __init__(self,txt_file="datasets/methane/train_ldm.txt", data_root="datasets/methane/data", **kwargs):
        super().__init__(txt_file, data_root,
                         **kwargs)


class methaneUnconditionalValidation(MethaneUnconditionaltBase):
    def __init__(self,txt_file="datasets/methane/val_ldm.txt", data_root="datasets/methane/data", flip_p=0.,ag_rate=0., **kwargs):
        super().__init__(txt_file, data_root,
